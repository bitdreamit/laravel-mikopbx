...
<div class="space-y-6" x-data x-init>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        ...
        <form method="POST" action="{{ route('mikopbx.callbacks.store') }}" class="grid grid-cols-2 md:grid-cols-5 gap-3">
            @csrf
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="divide-y divide-gray-50 max-h-96 overflow-y-auto">
                    ...
                    <div class="px-4 py-3 flex items-center gap-3">
                        <span class="w-2 h-2 rounded-full flex-shrink-0 {{ $cb->status === 'completed' ? 'bg-green-400' : 'bg-gray-300' }}"></span>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-900 truncate">{{ $cb->name ?? $cb->number }}</p>
                            <p class="text-xs text-gray-400">{{ $cb->number }} • {{ ucfirst($cb->status) }}</p>
                        </div>
                ...
                <div class="px-4 py-2 border-t border-gray-100 text-xs">{{ $completed->appends(['pending_page' => request('pending_page')])->links() }}</div>
                @endif
            </div>
        </div>
    </div>

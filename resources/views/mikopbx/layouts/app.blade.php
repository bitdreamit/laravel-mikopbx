    ...
    <script>
        ...
        function mikopbxApp() {
            return {
                ...
                incomingCall:     false,  // true while an unanswered inbound call is ringing
                    ...
                    try {
                        ...
                        window._mikopbxUA.on('newRTCSession', (e) => {
                            ...
                            if (e.originator === 'remote') {
                                ...
                                this.incomingCall = true;   // shows the Answer/Reject popup
                                ...
                                e.session.on('failed', () => { this.incomingCall = false; });
                            }
                        });
                ...
                makeCall() {
                    ...
                    if (this.sipRegistered && window._mikopbxUA) {
                        try {
                            const host   = window._mikopbxUA.configuration?.uri?.host ?? '{{ config("mikopbx.dialer.sip_server") }}';
                ...
                answerCall() {
                    try {
                        window._mikopbxSession?.answer({ mediaConstraints: { audio: true, video: false } });
                        this.incomingCall = false;
                        this.inCall = true;
                        this._callSec  = 0;
                        this.callTimer = '00:00';
                        ...
                        console.error('[MikoPBX Dialer] Answer failed:', err);
                    }
                },
...
<div x-show="incomingCall" x-cloak
     class="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-80"
    ...
    <div class="bg-white rounded-2xl shadow-2xl border-2 border-green-400 overflow-hidden">
        ...
        <div class="px-5 py-4 flex items-center gap-4">
            ...
            <div class="min-w-0">
                <p class="text-lg font-bold text-gray-900 font-mono truncate" x-text="incomingFrom"></p>
                <p class="text-xs text-gray-400">Calling extension</p>
            </div>
        </div>
        <div class="px-4 pb-4 grid grid-cols-2 gap-2">
            <button @click="answerCall()"
                    class="flex items-center justify-center gap-2 h-11 bg-green-500 hover:bg-green-600 rounded-xl text-white font-bold transition-colors">
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z"/></svg>
                Answer
            </button>
            <button @click="rejectCall()"
                    class="flex items-center justify-center gap-2 h-11 bg-red-500 hover:bg-red-600 rounded-xl text-white font-bold transition-colors">
...
{{-- Incoming call UI is now handled natively above via Alpine + JsSIP --}}

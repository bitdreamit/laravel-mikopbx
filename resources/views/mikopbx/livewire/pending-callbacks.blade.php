<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden" wire:poll.15s>
    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-50">
            ...
            <tr class="table-row">
                ...
                <td class="px-4 py-2">
                    <span class="badge {{ $cb->priority_badge }}">{{ ucfirst($cb->priority) }}</span>
                </td>
                <td class="px-4 py-2 text-xs text-gray-500">
                    {{ $cb->scheduled_at ? $cb->scheduled_at->format('d M H:i') : 'ASAP' }}
                    @if($cb->scheduled_at && $cb->scheduled_at->isPast())
                        <span class="text-red-500 font-medium"> (overdue)</span>
                    @endif
                </td>
                <td class="px-4 py-2 text-xs text-gray-500 max-w-[160px] truncate">{{ $cb->note ?? '—' }}</td>
                <td class="px-4 py-2">
                    <div class="flex items-center gap-2">
                        <button wire:click="attempt({{ $cb->id }})"
                                class="btn-success text-xs px-2 py-1">📞 Call</button>
                        <button wire:click="cancel({{ $cb->id }})"
                                wire:confirm="Cancel this callback?"
                                class="text-xs text-gray-400 hover:text-red-500 px-2 py-1 rounded hover:bg-red-50 transition-colors">✕</button>
                    </div>
                </td>
            </tr>
            @empty
            <tr><td colspan="5" class="px-4 py-10 text-center text-sm text-gray-400">No pending callbacks 🎉</td></tr>
            @endforelse
        </tbody>
    </table>
    @if($callbacks->hasPages())
    <div class="px-4 py-3 border-t border-gray-100">{{ $callbacks->links() }}</div>
    @endif
</div>
